...
public class Code01_SubsquenceMaxModM {
    ...
    //这是一个背包问题，要还是不要的问题
    //情况一要了这个数字加入子序列，sum+arr[index],index+1;情况二不要这个数字加入子序列,index+1;
    //先创建一个set，为了去重，把sum加入到set中
    //但是注意base case ，每次把sum加入到set中的时候，都要把arr所有元素都选过了，就是要不要，都选过到了。比如只要第0个，那就是后面的全部不要，但是也是要选过，选择的结果是不要而已。
    public static int max11(int[] arr, int m) {
        HashSet<Integer> hashSet = new HashSet<>();
        process11(arr,0,0,hashSet);
        int max =0;
        for(Integer sum:hashSet){
            max = Math.max(max,sum%m);
        }
        return max;
    }
    //index是下标
    //sum是子序列的和
    private static void process11(int[] arr, int index, int sum, HashSet<Integer> hashSet) {
        if(index==arr.length){
            hashSet.add(sum);
        }else{
            process(arr,index+1,sum+arr[index],hashSet);//要
            process(arr,index+1,sum,hashSet);//不要
        }
    }
    //dp[i][j],i是数量，j是子序列和
    //这个动态规划想不出来
